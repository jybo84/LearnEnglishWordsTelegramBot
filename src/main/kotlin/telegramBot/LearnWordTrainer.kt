...
class LearnWordTrainer() {
    ...
    fun getNextQuestion(): Question? {
        val notLearnedList = dictionary.filter { it.correctAnswersCount < MAX_LIST_WORD_FOR_USER }
        val remainsWord = dictionary.filter { it.correctAnswersCount < LIMIT_OF_LEARNED_WORD }
        if (remainsWord.isEmpty()) { return null
        val questionWords = notLearnedList.take(LIMIT_OF_LEARNED_WORD).shuffled()
        }
        val questionWords = remainsWord.shuffled().take(MAX_LIST_WORD_FOR_USER)
